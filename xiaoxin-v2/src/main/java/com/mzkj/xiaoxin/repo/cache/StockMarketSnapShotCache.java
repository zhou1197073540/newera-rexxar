package com.mzkj.xiaoxin.repo.cache;
...
public class StockMarketSnapShotCache {
    ...
    public HashSet<String> getFullCodes() {
        try (Jedis jedis = RedisUtil.getJedis()) {
            final Set<String> hkeys = jedis.hkeys(Consts.STOCK_LATEST);
            return (HashSet<String>) hkeys;
        }
    }
    ...
    public HashSet<String> getCodes() {
        try (Jedis jedis = RedisUtil.getJedis()) {
            final Set<String> hkeys = jedis.hkeys(Consts.STOCK_LATEST);
            return hkeys.stream().map(
                    key -> key.replaceAll("sh|sz", "")
    ...
    private StockMarketSnapshotPO toSnapShotPO(String snapShotStr) {
        try {
            final StockMarketSnapshotPO stockMarketSnapshotPO
                    = jacksonMapper.readValue(snapShotStr, StockMarketSnapshotPO.class);
            return stockMarketSnapshotPO;
        } catch (Exception e) {
            return null;
        }
